...
In a web browser, open the Microsoft Foundry portal at https://ai.azure.com to start building; signing in using the username User1-63800150@LODSPRODMCA.onmicrosoft.com and the password xNS5gdFc. Close any tips or quick start panes that are opened the first time you sign in.

If it is not already enabled, in the tool bar at the top of the page, enable the New Foundry option. Then, if prompted, create a new project with the project name Project63800150; expanding the Advanced options area to specify the following settings for your project:

Foundry resource: Project63800150-resource
...
In the model catalog, search for gpt-5.2.
Review the model card, and then deploy it using the default settings.
When the model has been deployed, it will open in the model playground.
...
After deploying your model, you should be in the playground with that model selected. If not, select Build in the top menu bar, then select Deployments on the left, and then select the model you deployed.

In the model playground, in the pane on the left, ensure that your gpt-5.2 model is selected.

In the Instructions field, enter the following prompt:

TypeCopy
You are a travel assistant that provides information on travel services available from Margie's Travel.
In the chat pane, enter the query What are some recommended tourist activities in New York next month? and review the response.
...
In the pane on the left, under the instructions, in the Tools section, select Add and add the web_search tool.

In the chat pane, enter the same query What are some recommended tourist activities in New York next month? and review the response.
...
On the menu bar, select the Home page.

Note the Azure OpenAI Endpoint displayed there.
...
Open the command palette (Ctrl+Shift+P) and use the Git:clone command to clone the https://github.com/microsoftlearning/mslearn-ai-studio repo to a local folder (it doesn't matter which one). Then open it.
...
In the Explorer pane, navigate to the folder containing the application code files at /labfiles/tools/python/tools-app. The application files include:
...
In the Explorer pane, right-click the tools-app folder containing the application files, and select Open in integrated terminal (or open a terminal in the Terminal menu and navigate to the /labfiles/tools/python/tools-app folder.)

Note: Opening the terminal in Visual Studio Code will automatically activate the Python environment. You may need to enable running scripts on your system.

Ensure that the terminal is open in the /labfiles/tools/python/tools-app folder with the prefix (.venv) to indicate that the Python environment you created is active.

Install the OpenAI SDK, Azure identity, and other required packages by running the following command:

TypeCopy
pip install -r requirements.txt
In the Explorer pane, in the /labfiles/tools/python/tools-app folder, select the .env file to open it. Then update the configuration values to include the Azure OpenAI Endpoint and the name assigned to the deployment for the gpt-5.2 model.
...
In the Explorer pane, in the /labfiles/tools/python/tools-app folder, select the tools-app.py file to open it.
...
In the main function, note that code to load the endpoint and key from the configuration file has already been provided. Then find the comment Initialize the OpenAI client, and add the following code to create a client for the OpenAI API:
...
In the main function, find the comment Create vector store and upload files, and add the following code:
...
In the main function, note that code to request a user prompt until the user quits the app has been provided. Within this loop, find the Get a response using tools comment, and add the following code:
...
When prompted, enter What's happening in San Francisco next month? and review the response from your generative AI model.
