...
Instructions
Resources
When creating a project using the New Foundry setting, ensure you uncheck the option to Set up recommended resources so I can explore everything Foundry has to offer and create resources following the instructional steps:
...
Select ResourceGroup1 as the resource group, and enter ai-agents-project63842939 for your Foundry project name to create the project for this exercise.
...
In the Model Catalog, locate the gpt-5 model (you can use the search bar to find it quickly).

Select Deploy next to the gpt-5 model.
...
If you are unable to identify the proper Endpoint in the VS Code Extensions, it can be found in the Foundry Portal https://ai.azure.com/ in the Microsoft Foundry project endpoint field.
...
Find the comment Add an inventory check mcp tool and add the following decorator above the function definition, which should now look like this:
...
Find the comment Add a weekly sales mcp tool and add the following decorator above the function definition, which should now look like this:
